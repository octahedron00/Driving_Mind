...
class Control_Mind:
    ...
    def __init__(self, show_function = showing_off, keyboard_listener = keyboard):
        ...
        if IS_LOG_VID:
            self.logwriter = cv2.VideoWriter(f"vlog_control_{now}.avi", cv2.VideoWriter_fourcc(*'MP4V'), 10.0, (640, 480))
    ...
    def action(self, frame):
        ...
        if self.keyboard_listener.is_pressed('w'):
            ...
            cv2.imwrite(os.path.join("img", f"{self.image_name}_{self.image_count:04d}.jpg"), frame)
            self.image_count += 1
